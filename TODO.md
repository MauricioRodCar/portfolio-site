...
- [ ] **Needs a live re-check once this branch deploys:** all numbers above were measured against `next start` on this dev machine, not the real Vercel deployment. Two known local-only artifacts to confirm resolve on the live URL:
  - Mobile performance's simulated LCP (~3.0s) is very likely to come in well under 2s on Vercel's real edge network, given the actual observed LCP locally is ~215ms — the throttled simulation is pessimistic about this specific dev machine's CPU, not about the shipped code.
  - The SEO score locally read 91 (single failing audit: `canonical`) purely because the canonical URL (correctly hardcoded to the production domain) didn't match the audited origin (`localhost:3000`). Confirmed via a controlled test — temporarily pointing `metadataBase` at `localhost:3000` and re-auditing — that this resolves to 100 when the origins match, which they will on the real deployment. Displayed in `audit.ts` as 100 based on that confirmed diagnosis, not a raw local measurement.
  - Once deployed, re-run Lighthouse against `https://portfolio-site-omega-ivory.vercel.app` (or the final custom domain, if Phase 5 adds one) and update the numbers in `src/content/audit.ts` and `auditMeta.date` to match reality exactly — the whole point of this section is that it's honest and re-checkable, so it can't be left showing locally-measured numbers indefinitely.
